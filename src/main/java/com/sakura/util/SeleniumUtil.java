...
public class SeleniumUtil {
	...
	public static WebElement getElement(TestStep step) throws Exception {
		...
		try {
			...
			if (step.getSkip() == null || StringUtil.isNotEqual("true", step.getSkip())) {
				log.error("『发现问题』执行异常: " + "<" + step.getId() + "." + step.getName() + "==> 未找到元素：【" + locatevalue + "】");
		...
		for (int i = 0; i < s.length; i++) {
			...
			if (i == s.length - 1) {
				...
				try {
					...
					// e.printStackTrace();
					log.error("对应的值不是键值对集合，无法继续获取值！", e);
